...
*   **Parsing and validation**
    - **Tokenization**: Splits input lines into tokens, treating commas as separators.
    - **Label validation**: Ensures labels follow the required syntax and are case-sensitive.
    - **Numeric parsing**: Supports decimal and hexadecimal values (`0x...`) and masks values to the 20-bit word width.
    - **Comment handling**: Uses `#` as the comment delimiter. Ensures comments only appear after the required operands for each line type.
...
*   **Output**
    - Produces a `memin.txt` file containing exactly `MEM_SIZE` lines.
